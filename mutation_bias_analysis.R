...
# file <- "all_mink.audacity_only.v8_masked.aln.fasta"
# meta_file <- "all_mink.tsv"

# human_background <- "V1"
# host_name <- "Deer"
# species_name <- "Odocoileus virginianus"
# file <- "deer.USA.n6670.audacity_only.v8_masked.aln.fasta"
# meta_file <- "deer.USA.n6670.tsv"
...
# human_background <- "V5"
# host_name <- "Mink"
# species_name <- species_name <- "Neovison vison"
# file <- "mink.USA.n258.audacity_only.v8_masked.fasta"
# meta_file <- "mink.USA.n258.csv"
...
get_mutation_freqs <- function(aln, species_name, meta.match) {
  ...
  morsels <- foreach(pos = seq(ncol(aln)), .packages = c("ape", "tidyverse")) %dopar% {
    ...
    tibble(pos = pos,
           ...
           base_freq = as.numeric(base_freq) / n_isolates)
  }
...
# 
# perm_df %>%
#   filter(nuc_trans == "A->G") %>%
#   summarise(sum(log_diff < mut_obs$log_diff) / 1000)
...
plot_df %>%
  ...
  ggplot(aes(x = nuc_trans, y = count, fill = nuc_trans)) +
