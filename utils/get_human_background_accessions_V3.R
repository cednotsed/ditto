...
for (country in countries) {
  ...
  # Get human isolates that are in same country and same time span
  in_same_country <- all_meta %>% 
    filter(host %in% c("Human")) %>%
    filter(grepl(country, location)) %>%
    filter(collection_date >= min(animal_filt$collection_date) & 
             collection_date <= max(animal_filt$collection_date))
  ...
  # table(human_filt$pango_lineage)
  ...
  fwrite(human_animal,
         str_glue("data/metadata/human_animal_subsets/V3/{host_name}.{country_name}.n{nrow(accessions)}.csv"))
  
  fwrite(dates,
         str_glue("data/metadata/human_animal_subsets/V3/{host_name}.{country_name}.n{nrow(accessions)}.dates_only.csv"))
  
  fwrite(accessions,
         str_glue("data/metadata/human_animal_subsets/V3/{host_name}.{country_name}.n{nrow(accessions)}.accessions_only.csv"))
}
