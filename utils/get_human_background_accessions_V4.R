...
for (country in countries) {
  ...
  human <- all_meta %>% 
    ...
    filter(pango_lineage %in% unique(animal$pango_lineage))
  ...
  fwrite(human_animal,
         str_glue("data/metadata/human_animal_subsets/V4/{host_name}.{country_name}.n{nrow(accessions)}.csv"))
  
  fwrite(dates,
         str_glue("data/metadata/human_animal_subsets/V4/{host_name}.{country_name}.n{nrow(accessions)}.dates_only.csv"))
  
  fwrite(accessions,
         str_glue("data/metadata/human_animal_subsets/V4/{host_name}.{country_name}.n{nrow(accessions)}.accessions_only.csv"))
}
